...
class VideoPage(PageObject):
    ...
    def captions_text(self):
        ...
        #
        # def _wait_for_captions_text():
        #     """
        #     Extract captions text.
        #     """
        #     return self.q(css=captions_css).text[0] != ""
        #
        # EmptyPromise(_wait_for_captions_text, 'Captions Text').fulfill()
        #
        # return self.q(css=captions_css).text[0]
    ...
    def _wait_for_browser_resize(self, old_size, new_width, new_height):
        """
        Wait until browser resize
        :param old_size: dict, old size of browser
        :param new_width: int
        :param new_height: int
        :return: bool
        """
        def _is_browser_resized():
            return self.browser.get_window_size() != old_size if old_size["width"] != new_width and old_size[
                "height"] != new_height else True

        EmptyPromise(_is_browser_resized, 'Browser Re-sized').fulfill()
    ...
    def is_aligned(self, is_transcript_visible):
        ...
        self._wait_for_browser_resize(initial, 300, 600)
        ...
        old_size = {'width': 300, 'height': 600}
        self._wait_for_browser_resize(old_size, 600, 300)
        real, expected = self._get_dimensions()
